#!/usr/bin/env python3
"""
Regenerate Audio Script for Kaldi's Coffee - Single Serve (Corrected Timing)
Generates enhanced audio narration with correct timing.
"""
...
class KaldiAudioGeneratorCorrected:
    def __init__(self):
    ...
    def enhance_kaldi_brewing_steps(self, recipe: Dict[str, Any]) -> List[str]:
        """Generate enhanced brewing steps for Kaldi's Coffee - Single Serve with correct timing."""
        params = recipe.get("parameters", {})
        ...
        total_time = params.get("total_brew_time_seconds", 180) / 60
        
        enhanced_steps = [
            f"Start your timer now. It's time to start the bloom phase. Pour {bloom_water} milliliters of water into the center divot, then move in a circular motion to the outside grounds. This initial pour should saturate all the coffee evenly and create a consistent slurry. The bloom will last for {bloom_time} seconds.",
            ...
            f"Immediately after reaching {total_water} milliliters, give one final swirl to flatten the bed for even drain down. This final step ensures consistent extraction and a clean finish. The total brew time should be around {total_time:.1f} minutes. Take a moment to appreciate the aroma as the coffee finishes extracting."
        ]
        
        return enhanced_steps
    
    def generate_kaldi_audio(self, recipe: Dict[str, Any], output_dir: str) -> None:
        """Generate enhanced audio for Kaldi's Coffee - Single Serve with correct timing."""
        title = recipe.get('title', 'Kaldi\'s Coffee - Single Serve')
        print(f"\n🎵 Generating corrected audio for: {title}")
        ...
        print(f"🎉 Corrected audio generation complete for Kaldi's Coffee - Single Serve!")
...
def main():
    """Main function to generate corrected audio for Kaldi's Coffee - Single Serve."""
    print("🚀 Corrected Audio Generator for Kaldi's Coffee - Single Serve")
    print("=" * 70)
    
    # Initialize generator
    generator = KaldiAudioGeneratorCorrected()
    ...
    print("\n🎉 Corrected audio generation complete!")
