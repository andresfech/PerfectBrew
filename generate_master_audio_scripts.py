#!/usr/bin/env python3
"""
Master Audio Script Generator for PerfectBrew
...
class MasterAudioScriptGenerator:
    def __init__(self):
        ...
        # Master narration templates with specific technique guidance
    ...
    def generate_master_audio(self, recipe: Dict[str, Any], output_dir: str) -> None:
        """Generate master audio for a single recipe."""
        recipe_title = recipe.get("title", "Unknown Recipe")
        print(f"\n🎵 Generating master audio for: {recipe_title}")
        ...
        print(f"🎉 Master audio generation complete for {recipe_title}!")
...
def main():
    """Main function to generate master audio for recipes."""
    print("🚀 Master Audio Script Generator for PerfectBrew")
    print("=" * 60)
    
    # Initialize generator
    generator = MasterAudioScriptGenerator()
    ...
    for recipe in recipes:
        ...
        output_dir = f"PerfectBrew/Resources/Audio/V60/{folder_name}_Master"
        
        try:
            generator.generate_master_audio(recipe, output_dir)
        except Exception as e:
            print(f"❌ Error processing {recipe_title}: {e}")
            continue
    
    print("\n🎉 Master audio generation complete for all V60 recipes!")
